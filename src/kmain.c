...
void kmain() {
  ...
  if(read_mhartid() == 0) {
    ...
    state0.sp = (reg_t)((usize)alloc_mem() + PAGE_SIZE - 1);

    state1.pp = MACHINE_MODE;
    state1.pc = (reg_t)&task1_func;
    state1.sp = (reg_t)((usize)alloc_mem() + PAGE_SIZE - 1);

    state2.pp = SUPERVISOR_MODE;
    state2.pc = (reg_t)&task2_func;
    state2.sp = (reg_t)((usize)alloc_mem() + PAGE_SIZE - 1);
    ...
    t0.page_table = (ptb_t*)zalloc_mem();
    t2.page_table = (ptb_t*)zalloc_mem();

    u8 cflags = PAGE_VALID | PAGE_READ | PAGE_WRITE | PAGE_EXECUTE;

    /* map t0 */
    /* map text segment */
    map_range(t0.page_table, &kernel_mapping_start, &kernel_mapping_start, (usize)&kernel_mapping_end - (usize)&kernel_mapping_start, cflags | PAGE_USER);
    /* map uart */
    ...
    /* activate satp for t0 */
    satp_page(t0.page_table);
    /* run t0 */
    switch_task(&t0.cpu_state);
  }
